...
// Disable caching for profile pages to always show the latest data
...
export default async function ProfilePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const completedEnrollments = await prisma.userCourse.count({
    where: {
      userId: Number(id),
      courseStatus: "COMPLETED", // Filter only completed courses
    },
  });
  ...
  const userCourses = await prisma.userCourse.findMany({
    ...
    include: {
      course: true, // Fetch the full course details
    },
  });
          ...
          <div className="grid gap-4">
            <div className="flex items-center justify-between">
              ...
              <div className="flex justify-center items-center gap-4 mt-8">
                <Image className="rounded-xl" src={"/profile-page/course.webp"} alt="Course Details" width={150} height={150}></Image>
                <div className="grid gap-2">
                  <h3 className="text-3xl font-semibold tracking-wide">UI & UX</h3>
                  <div className="flex gap-2 items-center text-sm text-gray-text font-light">
                    <p>START DATE: <span className="text-primary font-bold">Feb, 8, 2020</span></p>
                    <p>|</p>
                    <p>END DATE: <span className="text-primary font-bold">Mar, 8, 2020</span></p>
                  </div>
                </div>
              </div>
              ...
              <div className="flex gap-4 items-center">
                <div className="flex items-center gap-2">
                  ...
                  <h3 className="text-text text-4xl font-semibold">8.3</h3>
                </div>

                <button className="cursor-pointer bg-primary transition hover:bg-primary/80 p-5 px-8 text-white font-sm rounded-lg">VIEW COURSE</button>
              </div>
